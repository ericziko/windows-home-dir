...
let AppComponent = AppComponent_1 = class AppComponent {
            ...
            switch (event.type) {
                ...
                case "resultSet":
                    ...
                    let dataSet = {
                        ...
                        maxHeight: maxHeight,
                        minHeight: minHeight,
                        dataRows: new angular2_slickgrid_1.VirtualizedCollection(self.windowSize, resultSet.rowCount, loadDataFunction, (index) => {
                            return { values: [] };
                        }),
    ...
    openContextMenu(event, batchId, resultId, index) {
        ...
        if (contextMenuCell || grid.canCellBeActive(contextMenuCell.row, contextMenuCell.cell)) {
            if (selection.length === 0 ||
                ...
                selection = [
                    new angular2_slickgrid_1.SlickGrid.Range(contextMenuCell.row, contextMenuCell.cell - 1, contextMenuCell.row, contextMenuCell.cell - 1),
                ];
    ...
    isContextMenuCellWIthinSelection(selections, contextMenuCell) {
        for (const selection of selections) {
            if (selection.fromRow <= contextMenuCell.row &&
    ...
    onScroll(scrollTop) {
        ...
        this.scrollTimeOut = setTimeout(() => {
            if (self.dataSets.length < self.maxScrollGrids) {
                ...
                let numOfVisibleGrids = Math.ceil(tabHeight / gridHeight + (scrollTop % gridHeight) / gridHeight);
